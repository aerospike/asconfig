...
function build_container() {
  ...
  docker build --build-arg=PKG_VERSION="$PKG_VERSION" --build-arg=JF_USERNAME="$JF_USERNAME" --build-arg=JF_TOKEN="$JF_TOKEN" --progress=plain -t asconfig-pkg-tester-"$1":"$PKG_VERSION" -f .github/docker/test/Dockerfile-"$1" .
  docker tag asconfig-pkg-tester-"$1":"$PKG_VERSION" asconfig-pkg-tester-"$1":"latest"
}


function execute_build_image() {
  export BUILD_DISTRO="$1"
  docker run -e BUILD_DISTRO -v $(realpath ../dist):/tmp/output asconfig-pkg-tester-"$BUILD_DISTRO"-"$(git rev-parse HEAD | cut -c -8)"
}
RUN_TESTS=false
INSTALL=false
