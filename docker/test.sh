...
# docker/test.sh
#
# Smoke-test locally-built (or pullable) Aerospike asconfig Docker images.
#
...
Smoke-test Aerospike asconfig Docker images.

REQUIRED:
    -v, --version VERSION   Expected asconfig version string, e.g. 0.20.1 or 0.20.1-rc1
...
EXAMPLES:
    ...
    docker/test.sh -v 0.20.1-rc1

    # Test all arches for all distros
    docker/test.sh -v 0.20.1 -a amd64 -a arm64

    # Test a specific image tag
    docker/test.sh -i aerospike/aerospike-asconfig:0.20.1-ubuntu24.04-amd64 -v 0.20.1

    # Test + Snyk scan + cleanup
    docker/test.sh -v 0.20.1 -s -c
EOF
}
...
function test_image() {
  ...
  # Smoke test: run asconfig --version
...
function run_snyk() {
  ...
  local scan_log="aerospike-asconfig-${version}-${distro:+${distro}-}${arch}-${ts}.snyk"
