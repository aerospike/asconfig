...
TAGS PRODUCED:
  Test mode (loaded locally):
    <reg>/aerospike-asconfig:<version>-<arch>
    e.g. aerospike/aerospike-asconfig:5.0.0-amd64

  Push mode — multi-arch (default, both arches):
    <reg>/aerospike-asconfig:<version>
    <reg>/aerospike-asconfig:<version>-<timestamp>   (when -T is set)
...
function run_manifest_mode() {
  ...
  for reg in "${REGISTRY_PREFIXES[@]}"; do
    ...
    src_amd64="${reg}/aerospike-asconfig:${VERSION}-amd64"
    src_arm64="${reg}/aerospike-asconfig:${VERSION}-arm64"
    target_tag="${reg}/aerospike-asconfig:${VERSION}"
...
function generate_bake() {
...

  {
    ...
    for arch in "${ACTIVE_ARCHES[@]}"; do
      ...
      local tags=()
      for reg in "${REGISTRY_PREFIXES[@]}"; do
        tags+=("${reg}/aerospike-asconfig:${VERSION}-${arch}")
      done
    ...
    if [[ "${PUSH_SINGLE_ARCH:-false}" == true ]]; then
      ...
      local tags=()
      for reg in "${REGISTRY_PREFIXES[@]}"; do
        tags+=("${reg}/aerospike-asconfig:${VERSION}")
        if [[ -n "${TIMESTAMP}" ]]; then
          tags+=("${reg}/aerospike-asconfig:${VERSION}-${TIMESTAMP}")
        fi
      done
