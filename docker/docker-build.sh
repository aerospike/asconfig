...
#
# Build multi-architecture Docker images for Aerospike asconfig.
#
# Flow:
...
Build the Aerospike asconfig Docker image (single recipe; multi-arch).
...
REQUIRED:
    -v, --version VERSION   asconfig version, e.g. 0.20.1 or 0.20.1-rc1
...
TAGS PRODUCED:
  ...
  Test mode (loaded locally):
    <reg>/aerospike-asconfig:<tag>-<arch>
    e.g. aerospike/aerospike-asconfig:5.0.0-1-amd64

  Push mode — multi-arch (default, both arches):
    <reg>/aerospike-asconfig:<tag>
    <reg>/aerospike-asconfig:<tag>-<timestamp>   (when -T is set)
...
EXAMPLES:
    ...
    # Build + push to default registry (docker.io/aerospike/aerospike-asconfig)
...
function _emit_test_target() {
  ...
  if [[ "${arch}" == "amd64" ]]; then
    _emit_args \
      ASCONFIG_AMD64_URL "${ASCONFIG_AMD64_URL}" \
      ASCONFIG_AMD64_SHA256 "${ASCONFIG_AMD64_SHA256}" \
      ASCONFIG_LOCAL_PKG "${local_pkg}"
  else
    _emit_args \
      ASCONFIG_ARM64_URL "${ASCONFIG_ARM64_URL}" \
      ASCONFIG_ARM64_SHA256 "${ASCONFIG_ARM64_SHA256}" \
      ASCONFIG_LOCAL_PKG "${local_pkg}"
  fi
  _emit_tags "${tags[@]}"
  echo "}"
...
function _emit_push_target() {
  ...
  _emit_args \
    ...
    ASCONFIG_ARM64_URL "${ASCONFIG_ARM64_URL}" \
    ASCONFIG_ARM64_SHA256 "${ASCONFIG_ARM64_SHA256}" \
    ASCONFIG_LOCAL_PKG_AMD64 "${local_pkg_amd64}" \
    ASCONFIG_LOCAL_PKG_ARM64 "${local_pkg_arm64}"
  _emit_attestations
  _emit_tags "${tags[@]}"
  echo "}"
  echo ""
...
function _emit_single_arch_push_target() {
  ...
  if [[ "${arch}" == "amd64" ]]; then
    _emit_args \
      ASCONFIG_AMD64_URL "${ASCONFIG_AMD64_URL}" \
      ASCONFIG_AMD64_SHA256 "${ASCONFIG_AMD64_SHA256}" \
      ASCONFIG_LOCAL_PKG_AMD64 "${local_pkg}"
  else
    _emit_args \
      ASCONFIG_ARM64_URL "${ASCONFIG_ARM64_URL}" \
      ASCONFIG_ARM64_SHA256 "${ASCONFIG_ARM64_SHA256}" \
      ASCONFIG_LOCAL_PKG_ARM64 "${local_pkg}"
  fi
  _emit_attestations
  _emit_tags "${tags[@]}"
  echo "}"
...
function run_manifest_mode() {
  ...
  for reg in "${REGISTRY_PREFIXES[@]}"; do
    ...
    src_amd64="${reg}/aerospike-asconfig:${IMAGE_TAG}-amd64"
    src_arm64="${reg}/aerospike-asconfig:${IMAGE_TAG}-arm64"
    target_tag="${reg}/aerospike-asconfig:${IMAGE_TAG}"
    log_info "imagetools create ${target_tag}"
    local create_args=(-t "${target_tag}")
    if [[ "${MOVING_TAGS}" == true ]]; then
      for mt in $(moving_tags); do create_args+=(-t "${reg}/aerospike-asconfig:${mt}"); done
    fi
...
#                ASCONFIG_*_URL, ASCONFIG_*_SHA256, LOCAL_PKG_*
# ---------------------------------------------------------------------------
function generate_bake() {
...

  {
    ...
    for arch in "${ACTIVE_ARCHES[@]}"; do
      ...
      local tags=()
      for reg in "${REGISTRY_PREFIXES[@]}"; do
        tags+=("${reg}/aerospike-asconfig:${IMAGE_TAG}-${arch}")
      done
    ...
    if [[ "${PUSH_SINGLE_ARCH:-false}" == true ]]; then
      ...
      local tags=()
      for reg in "${REGISTRY_PREFIXES[@]}"; do
        tags+=("${reg}/aerospike-asconfig:${IMAGE_TAG}")
        if [[ -n "${TIMESTAMP}" ]]; then
          tags+=("${reg}/aerospike-asconfig:${IMAGE_TAG}-${TIMESTAMP}")
        fi
        if [[ "${MOVING_TAGS}" == true ]]; then
          for mt in $(moving_tags); do tags+=("${reg}/aerospike-asconfig:${mt}"); done
        fi
      done
...
#   ASCONFIG_{AMD64,ARM64}_URL
#   ASCONFIG_{AMD64,ARM64}_SHA256
...
function resolve_packages() {
  local pkg_amd64="aerospike-asconfig_${PKG_VERSION}-${ITERATION}ubuntu24.04_amd64.deb"
  local pkg_arm64="aerospike-asconfig_${PKG_VERSION}-${ITERATION}ubuntu24.04_arm64.deb"
  local pool="${DEB_BASE_URL}/pool/${UBUNTU_CODENAME}/aerospike-asconfig"

  ASCONFIG_AMD64_URL="${pool}/${pkg_amd64}"
  ASCONFIG_ARM64_URL="${pool}/${pkg_arm64}"
  ...
  if [[ -n "${PACKAGES_DIR}" ]]; then
    ...
    if [[ "${need_amd64}" == true ]]; then
      ...
      if [[ -n "${path}" ]]; then
        log_info "  Found:  ${path}"
        ASCONFIG_AMD64_SHA256=$(sha256_of_file "${path}")
        ...
        ASCONFIG_AMD64_SHA256="PLACEHOLDER"
      fi
    fi
    if [[ "${need_arm64}" == true ]]; then
      ...
      if [[ -n "${path}" ]]; then
        log_info "  Found:  ${path}"
        ASCONFIG_ARM64_SHA256=$(sha256_of_file "${path}")
        ...
        ASCONFIG_ARM64_SHA256="PLACEHOLDER"
      fi
    fi
  elif [[ "${COMPUTE_SHA}" == true ]]; then
    [[ "${need_amd64}" == true ]] && ASCONFIG_AMD64_SHA256=$(sha256_of_url "${ASCONFIG_AMD64_URL}")
    [[ "${need_arm64}" == true ]] && ASCONFIG_ARM64_SHA256=$(sha256_of_url "${ASCONFIG_ARM64_URL}")
  else
    ASCONFIG_AMD64_SHA256="PLACEHOLDER"
    ASCONFIG_ARM64_SHA256="PLACEHOLDER"
  fi
  ...
  echo ""
  log_info "  AMD64 URL: ${ASCONFIG_AMD64_URL}"
  log_info "  AMD64 SHA: ${ASCONFIG_AMD64_SHA256}"
  log_info "  ARM64 URL: ${ASCONFIG_ARM64_URL}"
  log_info "  ARM64 SHA: ${ASCONFIG_ARM64_SHA256}"
  if [[ "${ASCONFIG_AMD64_SHA256}" == "PLACEHOLDER" || "${ASCONFIG_ARM64_SHA256}" == "PLACEHOLDER" ]]; then
...
ASCONFIG_AMD64_URL=""
ASCONFIG_AMD64_SHA256=""
ASCONFIG_ARM64_URL=""
ASCONFIG_ARM64_SHA256=""
